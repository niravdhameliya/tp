...
public class LabourInvoicePDF {
    ...
    public LabourInvoicePDF(LaborInvoiceDetails invoice_details) {
...
//        textt.add("M/S,\n" + UtilVars.getCustomer().getName() + "\n" + UtilVars.getCustomer().getAddress());
    ...
    private void addItemsData(Document document) throws DocumentException {
        ...
        for (int i = 0; i < itemList.size(); i++) {
            ...
            double total_amount = temp.getAmount();
            table.addCell(setItemsFont("Item Code:" + item_code + "\n" + items_name + "\nSAC: " + invoice_details.getSac()));
            table.addCell(setItemsFont(quant));
            table.addCell(setItemsFont(unit_rate));
            table.addCell(setItemsFont(String.format("%.2f", total_amount) + ""));
        }
        //PdfPCell cell1 = new PdfPCell(new Phrase(" "));
        //cell1.setBorder(Rectangle.BOX);
        String space = "\n\n\n";
        for (int d = 0; d < (5 - itemList.size()); d++) {
            space = space + "\n\n\n";
        }
        /*String cell1 = "\n\n";
    ...
    private void addFinalBlock(Document document) throws DocumentException {
        ...
        block2.setWidths(new int[]{55, 45});
        block2.addCell(setParaHeader("Basic Total"));
        block2.addCell(setParaHeader(String.format("%.2f", invoice_details.getBasicTotal()) + ""));//add basic total
        ...
        block2.addCell(setParaHeader(String.format("%.2f", taxAmnt) + ""));//add taxable amount
        block2.addCell(setParaHeader("SGST " + invoice_details.getSgst() + " %"));
        block2.addCell(setParaHeader("" + String.format("%.2f", calGst(invoice_details.getSgst(), taxAmnt))));//add sgst
        block2.addCell(setParaHeader("CGSI " + invoice_details.getCgst() + " %"));
        block2.addCell(setParaHeader("" + String.format("%.2f", calGst(invoice_details.getCgst(), taxAmnt))));//add cgst
        block2.addCell(setParaHeader("IGST " + invoice_details.getIgst() + " %"));
        block2.addCell(setParaHeader("" + String.format("%.2f", calGst(invoice_details.getIgst(), taxAmnt))));//add Igst
        //      block2.addCell(setParaHeader("Total"));
        //    block2.addCell(setParaHeader(""+invoice_details.getGrandTotal()));//add total
        block2.addCell(setParaHeader("Amount"));
        block2.addCell(setParaHeader(String.format("%.2f", invoice_details.getAmount()) + ""));//add basic total

        block2.addCell(setParaHeader("Round OFF"));
        block2.addCell(setParaHeader(String.format("%.2f", invoice_details.getRoundOFF()) + ""));//add basic total

        block2.addCell(setParaHeader("Grand Total"));
        block2.addCell(setParaHeader("" + String.format("%.2f", invoice_details.getGrandTotal())));//add grand Total
    ...
    public static void main(String[] args) {
        try {
            ...
            Logger.getLogger(LabourInvoicePDF.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
}
