...
import static com.mileset.neeleshengineers.View.Report.PriceBidPDF.companyName;
 ...
 * @author rahul
 */
public class TechnicalBidPDF{
String refData="Reference is made to the above mentioned Tender Enquiry, the details in respect of the Tender Enquiry are mentioned below:";
    ...
    //private String clientName, tender_enqNo, tender_enqDate;
    private String FILE = "D:\\NeeleeshEngineers\\technicalbid\\";
    String quantityType = " Nos.";
    private String website = "www.neeleshengineers.com";
    ArrayList<TechnicalBidItems> technicalBidItemsList;
    TechnicalBid technicalBid;

    public TechnicalBidPDF(TechnicalBid tb) {
        this.technicalBid = tb;
        this.billNo = this.technicalBid.getTenderEnqNo().replaceAll("/", "_");
        this.billNo = billNo.replaceAll("\\\\", "_");
        technicalBidItemsList = technicalBid.getTechnicalBidItems();
    ...
    private static void addMetaData(Document document) {
        document.addTitle("Technical Bid");
        document.addSubject("Technical Bid");
        document.addKeywords("Java, PDF, Technical Bid, Neelesh Engineers, Swapnil Gandhi");
    ...
    private void addTitlePage(Document document)
        ...
        tInfo.setFont(font3);
//        tInfo.add("\nEmail:" + mailid + "                                        PAN No.: ABJPP1674P");
    ...
    private void addClientInfo(Document document) throws DocumentException {
        ...
        textt.setAlignment(Element.ALIGN_CENTER);
        textt.setFont(font2);
        textt.add("Qtn.No:-" + technicalBid.getQtnNo() + "                                                              Date:" + technicalBid.getDate() + "\n\n\nTo,\n" + technicalBid.getClientName());
        ...
        p11.setFont(font2);
        p11.add("Subject: " + technicalBid.getSubject());//add here subject
        ...
        p11.setAlignment(Element.ALIGN_LEFT);
        p11.setFont(font2);
        p11.add("Reference: Tender Enquiry No: " + technicalBid.getTenderEnqNo() + "           Date: " + technicalBid.getTenderEnqDate()+"\n"+refData);//add here enq no & date
    ...
    private void addItemsData(Document document) throws DocumentException {
        ...
        float[] columnWidths = new float[]{5f, 40f, 15f, 35f};
        table.setWidths(columnWidths);

        for (int i = 0; i < technicalBidItemsList.size(); i++) {
            TechnicalBidItems temp = technicalBidItemsList.get(i);
            int n = i + 1;
            table.addCell(setItemsFont("" + n));
            String nomenclature = temp.getNomenclature();
            String quant = temp.getQuantity();
            String remarks = temp.getRemarks();

            table.addCell(setItemsFont(nomenclature));
            table.addCell(setItemsFont(quant));
            table.addCell(setItemsFont(remarks));
    ...
    private void addTermsAndConditions(Document document) throws DocumentException {
        ...
        PdfPCell cell22 = new PdfPCell(Para4TC("" + technicalBid.getPaymentTerms()));
        cell22.setBorder(Rectangle.RECTANGLE);
        PdfPCell cell23 = new PdfPCell(Para4TC("" + technicalBid.getGovtTaxes()));
        cell23.setBorder(Rectangle.RECTANGLE);
        PdfPCell cell24 = new PdfPCell(Para4TC("" + technicalBid.getDelPeriod()));
        cell24.setBorder(Rectangle.RECTANGLE);
        PdfPCell cell25 = new PdfPCell(Para4TC("" + technicalBid.getDelTerms()));
        cell25.setBorder(Rectangle.RECTANGLE);
        PdfPCell cell26 = new PdfPCell(Para4TC("" + technicalBid.getPFCharges()));
        cell26.setBorder(Rectangle.RECTANGLE);
        PdfPCell cell27 = new PdfPCell(Para4TC("" + technicalBid.getTransitInsu()));
        cell27.setBorder(Rectangle.RECTANGLE);
        PdfPCell cell28 = new PdfPCell(Para4TC("" + technicalBid.getEmd()));
        cell28.setBorder(Rectangle.RECTANGLE);
        PdfPCell cell29 = new PdfPCell(Para4TC("" + technicalBid.getValOffer()));
        cell29.setBorder(Rectangle.RECTANGLE);
        PdfPCell cell210 = new PdfPCell(Para4TC("" + technicalBid.getInstallComm()));
        cell210.setBorder(Rectangle.RECTANGLE);
        PdfPCell cell211 = new PdfPCell(Para4TC("" + technicalBid.getInspection()));
        cell211.setBorder(Rectangle.RECTANGLE);
        PdfPCell cell212 = new PdfPCell(Para4TC("" + technicalBid.getPSD()));
        cell212.setBorder(Rectangle.RECTANGLE);
        PdfPCell cell213 = new PdfPCell(Para4TC("" + technicalBid.getFIM()));
        cell213.setBorder(Rectangle.RECTANGLE);
        PdfPCell cell214 = new PdfPCell(Para4TC("" + technicalBid.getNote()));
    ...
    private void addSignature(Document document) throws DocumentException {
        ...
        textt.setAlignment(Element.ALIGN_LEFT);
        textt.setFont(font3);
        textt.add("Authorised Signature");
        document.add(textt);
    ...
    public static void main(String[] args) {
        try {
            ...
            Logger.getLogger(TechnicalBidPDF.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
